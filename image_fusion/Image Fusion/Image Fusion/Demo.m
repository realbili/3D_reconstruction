...
NameImg = 'E:\research\A_FOCUS_Data\4DLightFieldDataset_PSF\test\6\input\';                    % Name of the input image set
NumberOfImages = 100;                    % Number of images in the input set
Format = '.bmp';                       % The format or type of the input images

%% Preallocate stack where the images in the input set will be stored
tmp = imread(strcat(NameImg,'1',Format)); [s(1),s(2),s(3)]=size(tmp); clear tmp;
I = zeros([s,NumberOfImages]); clear s
%% Read the input images
for i = 1:NumberOfImages
    I(:,:,:,i) = imread(strcat(NameImg,'',num2str(i),Format));
end
...
imwrite(G,'E:\research\A_FOCUS_Data\4DLightFieldDataset_PSF\test\6\100\15_.bmp');
if size(I,3)==1
    imhist(uint8(G));
