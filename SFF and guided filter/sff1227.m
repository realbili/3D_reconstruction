function [z,R] = sff1227(imlist, varargin)
...
% 'fmeasure',   A string with the focus measure operator. (GLVM).          —— FM 评价名称_str
% 'filter',     A scalar with the size of median filter. (0).                             ——中值滤波尺寸
% 'interp',     A logic flag to turn Gaussian interpolation on/off (true)     ——高斯插值
% 'fsize',      An integer with the size of the focus mesure window. (9)    ——FM窗口大小
% 'focus',      A vector with the focus position of each image. (1:P)         ——聚焦位置？焦点位置？                  
% 'ROI',        Scene ROI as a rectangle [xo yo W H]. Default is [].            ——region of interest 感兴趣区域
%
% OUTPUTS:
% 
% Z,        is a MxN matrix with the reconstructed depthmap obtained using
%           SFF as originally proposed in [1]    ——初始深度图
% R,        is a MxN matrix with the reliability measure (in dB) of the
%           estimated depth computed as proposed in [2]   ——可靠性测量
%
% References:
...
opts = parseInputs(imlist, varargin{:});   %参数解析
...
for p = 1:P
    fm(:,:,p) = focusmeasure1227(im2double(images(:,:,p)),...
...
if opts.filter~=0
    fprintf('Smoothing ')
    z = medfilt2(real(z), [opts.filter opts.filter]);
    fprintf('[100%%]\n')
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Opts = parseInputs(imlist, varargin)                                  %输入参数分析
